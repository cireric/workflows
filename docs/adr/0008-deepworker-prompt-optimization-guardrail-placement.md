...
不可逆操作偏指令跟随（加结构化护栏），可逆操作偏 Hephaestus（信任模型自主判断）。
...
| PLAN | 可逆 | 偏 Hephaestus | 模型规划能力是强项，漂移检测已提供安全网 |
| EXECUTE 常规 | 可逆 | 偏 Hephaestus | TODO 铁律已提供基础纪律 |
...
- 理由：lint 修复是可逆操作，按原则应偏 Hephaestus；但行为/抑制类修复有隐蔽后果，需强制走决策树
...
**结论**：过度约束。lint 修复是可逆操作，按原则应偏 Hephaestus。
